...
if __name__ == '__main__':
    ...
    writeFile = '/data1/cby/temp/output_ff/logs/' + model_name
    store_name = '/data1/cby/temp/output_ff/weights/' + model_name
    if not os.path.isdir(store_name):
        os.makedirs(store_name)
    model_path = None
    # model_path = '/data1/cby/temp/output_ff/weights/efficientnet-b3/efn-b3_LS_17_acc_0.8014.pth'
    ...
    if is_train:
        ...
        for epoch in range(epoch_start, num_epochs):
            ...
            if acc > max_acc:
                max_acc = acc
                torch.save(model.state_dict(), '{}/efn-b2_LS_{}_acc_{:.4f}.pth'.format(store_name, epoch, acc))
            print('Current max acc:', max_acc)
        torch.save(model.state_dict(), '{}/efn-b2_LS_{}_acc_{:.4f}.pth'.format(store_name, 'last_50', acc))
